...
### ISSUE-021：开发可配置水平合成轨迹工具

- 状态：`DONE`
- 优先级：P1
- 目标：新增独立脚本 `scripts/loom_xy_motion.py`，在派生 ROS1 bag 中把已有动态 `odom → base_link` 的水平 `translation.x/y` 替换为用户明确指定方向、距离及起止时间的直线 minimum-jerk 轨迹，使 Foxglove 中的机器人从原位平滑移动、无回环地到达目标位置并停住，同时保持原有身体、手指及全部非水平平移数据。
- 依赖：ISSUE-020（已完成；真实开发和验收基线为 `test_output/issue-020/01_dexhand_tf.bag`）。
- 开发授权：用户于 2026-08-30 完成开发前确认，授权实现、测试、真实派生 bag、Foxglove 验收以及最终提交和推送。

#### 目标文件与范围

- 实现：`scripts/loom_xy_motion.py`。
- 测试：`tests/test_loom_xy_motion.py`。
- 测试和真实输出：仅允许写入 `test_output/issue-021/`。
- 配套文档：开发完成后同步 `README.md`、`.ai/DECISIONS.md`、`.ai/PROGRESS.md` 和 `CHANGELOG.md`。
- 输入 bag 始终只读；输出为独立派生 bag。输入和输出可以位于同一目录，但冲突处理后的最终输出不得与输入解析为同一个文件。
- 第一版只处理单个输入 bag；不实现批处理、真实里程计恢复、足部接触重建、IK、视觉里程计、marker 或其他可视化 topic。

#### CLI 契约

- 以下六个参数全部必传，不通过交互补齐：
  - `--input BAG`：已存在的单个 ROS1 bag；
  - `--output BAG`：期望输出路径；
  - `--direction robot-right|robot-left|robot-up|robot-down`：相对机器人第一帧姿态的移动方向；
  - `--distance-m METERS`：有限正数，单位为米；
  - `--start-s TIME`：相对第一条有效目标 TF header 时间的开始时刻；
  - `--end-s TIME`：同一时间基准下的结束时刻。
- 可选参数仅保留 `--dry-run`；第一版不暴露 `--profile` 或 `--overwrite`。
- 时间接受十进制秒、`MM:SS[.fraction]` 和 `HH:MM:SS[.fraction]`，例如 `2.5`、`00:02.5` 和 `00:01:20.5`。
- 扫描实际目标 TF 后必须验证 `0 <= start < end <= target_tf_duration`；不得写死任何真实 bag 的起止时间。
- 参数缺失、方向不支持、距离非有限/非正、时间格式非法或时间范围越界时，在创建输出目录或临时文件前明确失败。

#### 方向和轨迹语义

- 局部方向固定定义为：`robot-up=(+1,0)`（`base_link +x`，前）、`robot-down=(-1,0)`（后）、`robot-left=(0,+1)`（`base_link +y`，左）、`robot-right=(0,-1)`（右）。
- 从第一条有效 `odom → base_link` 四元数提取初始 yaw，仅用该 yaw 把选定局部单位向量旋转到 `odom`；不得随每帧 yaw 改变方向，否则轨迹会弯曲。
- 设用户时间归一化为 `u=clip((t-start)/(end-start),0,1)`，内部固定使用 minimum-jerk 进度 `g(u)=10u^3-15u^4+6u^5`，输出水平位置为 `p_out(t)=p_initial+distance*g(u)*direction_odom`。
- minimum-jerk 是内部实现而非用户参数；它必须保证开始/结束位置准确，开始/结束速度和加速度为零，方向投影单调不减，横向偏差仅为数值误差。
- 开始前水平位置固定在第一帧位置；结束后固定在最终目标位置。原始 `odom → base_link.x/y` 除第一帧锚点外不参与合成轨迹。

#### 允许的交互

- 距离、方向、开始和结束时间不得交互输入；交互仅用于输出冲突处理和最终确认。
- 若 `--output` 已存在，禁止静默覆盖，逐个冲突文件询问：
  1. 切换该文件的输出目录，保持原文件名；
  2. 在当前输出目录重新给出文件名，省略 `.bag` 时自动补全；
  3. 中止。
- “重新命名”只接受文件名，不接受目录分隔符；“切换目录”只改变当前冲突文件的目录。新候选仍冲突时继续询问，直到得到未占用且不等于输入的路径或用户中止。
- 非 TTY 环境遇到输出冲突时明确失败，要求调用者重新指定 `--output`，不得猜测新路径。
- 最终打印输入哈希、解析后的输出、目标边/数量/时长、方向及 `odom` 向量、距离、起止时间、初始/最终坐标、理论最大速度和待创建目录，随后询问 `Proceed [Y/n]:`；空输入、`y` 或 `yes` 为确认，`n` 或 `no` 中止，默认值为 `Y`，EOF 安全中止。
- `--dry-run` 完成扫描、冲突解析、参数/轨迹计算和完整计划打印，但不得创建目录、临时文件或输出，也不进入正式写入确认。

#### Bag 修改和保真边界

- 唯一允许修改的 bag 字段是动态 `/tf` 中 `header.frame_id == "odom"` 且 `child_frame_id == "base_link"` 的 `transform.translation.x/y`。
- 不新增 topic、connection、消息、TF 边、marker 或 odometry；不修改目标 transform 的 `translation.z`、旋转、header、parent/child、记录时间，也不修改任何身体、腿、手臂、头部、手指、静态 TF、传感器、关节、图像或相机数据。
- 不含目标 transform 的记录必须复制原始序列化字节；含目标 transform 的 `TFMessage` 可以因替换 `x/y` 重新序列化，但其中所有非目标 transform 必须逐字段一致。
- topic、connection metadata、callerid、latching、消息/connection 顺序、消息数量及全部 header/record 时间戳必须保持一致。

#### 扫描、写出和失败安全

- 写出前只读扫描输入哈希、连接元数据、唯一目标发布连接、目标数量/时间范围、第一帧位置/四元数、有限值和时间单调性；重复目标发布、非有限值、时间回退或无有效目标时不得创建输出。
- 最终确认后再次核对输入 SHA-256，在解析后的输出目录创建唯一临时 bag，流式复制记录并只重写目标 `x/y`。
- 临时 bag 写完后必须重新读取，执行轨迹、字段保真、连接/计数和 TF 拓扑验证；验证成功且输入哈希仍一致后，才原子发布到最终路径。
- 输出冲突在规划阶段解决后，若正式发布前目标路径被其他进程创建，必须停止且不覆盖；失败时只删除本次精确创建的临时文件。

#### 开发与测试阶段

- [x] 阶段一：冻结纯函数接口、六个必传参数、时间解析、四方向映射、minimum-jerk 公式、交互文本和输出不变量；先编写对应失败测试。
- [x] 阶段二：实现只读 bag/TF 扫描、输入哈希、唯一目标发布权、有限值及时间门禁；验证全部失败路径不创建文件。
- [x] 阶段三：实现输出冲突的换目录/重命名循环、非 TTY 行为、最终默认 `Y` 确认和 dry-run；覆盖二次冲突、取消及 EOF。
- [x] 阶段四：实现连接保真复制、目标 `x/y` 重写、唯一临时 bag、写后回读、竞态保护和原子发布；完成最小 bag 集成测试。
- [x] 阶段五：在 `VelaLoom` 环境中对 ISSUE-020 基线先执行 dry-run，再在 `test_output/issue-021/` 生成独立真实输出并完成自动验证；需要 ROS1 原生检查时按规定使用已验证挂载的 `ros1_noetic` 容器。
- [x] 阶段六：在 Foxglove 以 `odom` 为 Fixed frame 人工验收起点、选定机器人方向、距离、无回环、平滑启停、终点保持以及身体/手指动作保真。
- [x] 阶段七：运行模块测试、全仓回归、脚本/测试语法检查、CLI help、真实数据保真、输入哈希和 `git diff --check`，再同步配套文档。

#### 验收标准

- [x] `scripts/loom_xy_motion.py` 为独立脚本，六个业务参数均必传，时间三种格式和四个机器人相对方向均有自动化测试。
- [x] 方向使用第一帧 yaw 固定转换；轨迹开始前停在起点、区间内严格直线且方向投影单调、结束后停在精确距离处，横向误差不超过 `1e-9 m`。
- [x] 输出冲突时可只为当前文件换目录或重命名并处理二次冲突；不存在静默覆盖，同目录输入输出、非 TTY、取消、EOF 和发布竞态行为明确且通过测试。
- [x] 只改变目标 `translation.x/y`；输入 SHA-256 不变，目标数量不变，`z`、旋转、header/record 时间、连接元数据和所有非目标数据通过字节级或字段级保真验证。
- [x] 输出消息/topic/connection 数量与顺序不变，唯一根仍为 `odom`，无环、无多 parent，现有身体和手指 TF 验证结果不退化。
- [x] Foxglove 中模型从原位沿用户选择的机器人自身方向平滑移动指定米数，无杂乱回环，结束后保持在目标位置，原有身体和手指动作继续播放。
- [x] 所有测试产物仅位于 `test_output/issue-021/`；模块测试、全仓回归、语法/CLI、真实 bag、ROS1（如需要）、Foxglove 和 `git diff --check` 全部通过后才能标记 `DONE`。

### ISSUE-022：研究步态重定向与全身 IK

- 状态：`TODO`
- 优先级：P1
- 目标：深入研究通过步态阶段识别、支撑脚约束、目标落脚点重定向、骨盆轨迹规划和全身逆运动学，在扩大机器人有效移动幅度时仍保持自然步态、脚底接触和关节运动一致性。
- 依赖：ISSUE-020（已完成，提供完整 TF 与关节运动基线）；与 ISSUE-021 分开研究，不把视觉趋势放大当作 IK 输入或既定前提。
- 研究边界：本 Issue 当前仅评估可观测性、算法架构、约束、风险和验证方法，不实现 IK 求解器、不修改现有脚本/URDF/bag、不输出重定向数据。
- 重点研究内容：
  - 从现有 TF、关节状态和可用传感器中评估左右脚接触、单/双支撑阶段和步态事件的可观测性；
  - 比较基于支撑脚锚定的根轨迹重建、落脚点缩放、骨盆/质心轨迹规划和全身 IK 等技术路线；
  - 定义脚底无滑移、地面高度、关节限位、速度/加速度连续性、左右对称、自碰撞和动态可行性约束；
  - 评估只做运动学重定向与引入动力学、接触力或优化求解的差异及依赖；
  - 设计可逐级验证的最小原型、测试夹具、真实数据指标和 Foxglove 人工验收方式。
- 验收标准：
  - [ ] 明确现有数据能否可靠支持步态阶段和支撑脚识别，并列出缺失输入；
  - [ ] 给出至少两种候选重定向/IK 架构及其自然度、复杂度和风险对比；
  - [ ] 定义输入输出契约、坐标系语义、约束集合和关键算法流程；
  - [ ] 给出脚滑、关节限位、轨迹连续性和动作自然度的自动与人工验收指标；
  - [ ] 形成推荐路线、阶段化研发计划和停止条件，获得用户确认后再进入正式实现。
