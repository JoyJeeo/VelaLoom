...
- 开发授权：本条目只冻结已确认方案，状态保持 `TODO`；在用户完成开发前确认并明确授权正式开发前，不得实现脚本、测试或派生 bag。
...
- [ ] 阶段一：冻结纯函数接口、六个必传参数、时间解析、四方向映射、minimum-jerk 公式、交互文本和输出不变量；先编写对应失败测试。
- [ ] 阶段二：实现只读 bag/TF 扫描、输入哈希、唯一目标发布权、有限值及时间门禁；验证全部失败路径不创建文件。
- [ ] 阶段三：实现输出冲突的换目录/重命名循环、非 TTY 行为、最终默认 `Y` 确认和 dry-run；覆盖二次冲突、取消及 EOF。
- [ ] 阶段四：实现连接保真复制、目标 `x/y` 重写、唯一临时 bag、写后回读、竞态保护和原子发布；完成最小 bag 集成测试。
- [ ] 阶段五：在 `VelaLoom` 环境中对 ISSUE-020 基线先执行 dry-run，再在 `test_output/issue-021/` 生成独立真实输出并完成自动验证；需要 ROS1 原生检查时按规定使用已验证挂载的 `ros1_noetic` 容器。
- [ ] 阶段六：在 Foxglove 以 `odom` 为 Fixed frame 人工验收起点、选定机器人方向、距离、无回环、平滑启停、终点保持以及身体/手指动作保真。
- [ ] 阶段七：运行模块测试、全仓回归、脚本/测试语法检查、CLI help、真实数据保真、输入哈希和 `git diff --check`，再同步配套文档。

#### 验收标准

- [ ] `scripts/loom_xy_motion.py` 为独立脚本，六个业务参数均必传，时间三种格式和四个机器人相对方向均有自动化测试。
- [ ] 方向使用第一帧 yaw 固定转换；轨迹开始前停在起点、区间内严格直线且方向投影单调、结束后停在精确距离处，横向误差不超过 `1e-9 m`。
- [ ] 输出冲突时可只为当前文件换目录或重命名并处理二次冲突；不存在静默覆盖，同目录输入输出、非 TTY、取消、EOF 和发布竞态行为明确且通过测试。
- [ ] 只改变目标 `translation.x/y`；输入 SHA-256 不变，目标数量不变，`z`、旋转、header/record 时间、连接元数据和所有非目标数据通过字节级或字段级保真验证。
- [ ] 输出消息/topic/connection 数量与顺序不变，唯一根仍为 `odom`，无环、无多 parent，现有身体和手指 TF 验证结果不退化。
- [ ] Foxglove 中模型从原位沿用户选择的机器人自身方向平滑移动指定米数，无杂乱回环，结束后保持在目标位置，原有身体和手指动作继续播放。
- [ ] 所有测试产物仅位于 `test_output/issue-021/`；模块测试、全仓回归、语法/CLI、真实 bag、ROS1（如需要）、Foxglove 和 `git diff --check` 全部通过后才能标记 `DONE`。
