...
## D016：ISSUE-021 前期研究采用 3 倍低频水平趋势增强

- 日期：2026-08-29
- 决策：可视化增强固定使用 `p_out = p_raw + 2 * (p_trend - p_trend_initial)`；以 100 Hz 等间隔分析、0.11 秒中值预处理和三阶零相位 Butterworth `0.25 Hz` 低通提取 `odom → base_link` 的水平趋势。只改变根变换的 `x/y`，不改变 `z`、旋转及全部后代相对 TF。
- 证据：真实基线中 0.05–0.35 Hz 主要承载整体水平移动，约 1.0–1.1 Hz 存在步态成分。原始坐标直接 3 倍会使残差 RMS、根加速度 P95 和轨迹长度约为原始 3 倍；推荐方案的水平可见范围为原始 1.92946 倍，根加速度 P95 为 0.998382 倍，支撑脚代理漂移 P95 为 24.3019 mm/0.25 秒。
- 输出边界：用户确认研究结论后明确授权生成 `test_output/issue-021/01_dexhand_tf_trend3x.bag` 作为一次性可视化派生结果；没有新增生产脚本，原始 bag/URDF 保持不变。输出不得用于定位、控制、训练或机器人性能定量评估。
- 停止条件：后续同类转换若不能同时满足水平可见范围至少 1.8 倍、根加速度 P95 不超过原始 1.10 倍、支撑脚代理漂移 P95 不超过 25 mm/0.25 秒，则退回纯 Foxglove 视觉增强。需要真实支撑脚固定时转入 ISSUE-022 的接触约束和全身 IK，不继续放宽 ISSUE-021 的滤波参数。

## D017：可配置水平合成轨迹使用首帧方向和 minimum-jerk

- 日期：2026-08-30
- 决策：`loom_xy_motion.py` 将 `robot-up/down/left/right` 固定解释为第一帧 `base_link` 的前/后/左/右，只使用第一条有效 `odom → base_link` 四元数的 yaw 转换到 `odom`；水平位置使用 `p_initial + distance*(10u^3-15u^4+6u^5)*direction_odom`，开始前保持起点、结束后保持终点。
- 数据边界：唯一允许变化的是动态 `/tf` 中目标边的 `translation.x/y`；消息、connection、topic、时间戳、目标 `z`/旋转、同消息非目标 transform 及全部非目标记录保持不变。输入只读，输出必须是独立派生 bag。
- 交互与发布：六个业务参数全部由 CLI 明确给出；交互只处理输出冲突和最终 `[Y/n]`。工具不提供 `--overwrite`，临时 bag 完整回读通过后使用同目录无覆盖原子发布；发布竞态必须失败并保留竞争者文件。
- 原因：首帧固定方向保证轨迹严格直线，minimum-jerk 保证位置准确及端点速度、加速度为零；不复用原始根节点水平残差可以消除短距离回环，同时完整保留身体、手指和传感器动作。
- 风险：这是可视化合成而非里程计恢复或接触一致重定向，可能产生脚底滑动；不得用于定位、控制、训练或机器人性能定量评估，物理一致需求转入 ISSUE-022。
