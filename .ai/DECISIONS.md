...
## D015：TF 验证使用 URDF 驱动模型与整组源状态匹配

- 日期：2026-08-29
- 决策：`validate_tf.py` 从必选 URDF 动态发现全部 joint 类型和几何，只允许 YAML/CLI 提供传感器数组索引到标量 joint name 的映射；不在程序中硬编码 Kuavo joint 数量、名称、层级或运动学参数。
- 时间匹配：每个完整 TF 时间点只选择同一条传感器消息，默认窗口为前 30 ms、后 5 ms；按全部映射 joint 的误差联合选取候选，不允许不同 joint 使用不同时间的源值。角度与线性误差分开统计，并额外输出按各自最大门限归一化的无量纲整体 RMS。
- 多自由度边界：planar/floating 验证拓扑、有限值和允许运动空间，但不接受单标量数组映射；无法验证其源数据时显式进入独立策略，不静默跳过。
- 安全与状态：bag 和 URDF 始终只读并在前后核对 SHA-256；默认仅输出终端报告，`--json-out` 显式指定时才原子写入且拒绝覆盖。结果区分 `PASS`、`PASS_WITH_WARNINGS`、`FAIL`，`--strict` 将告警提升为失败。
- 阈值：通用几何和匹配门限使用明确 SI 单位；角度与线性限位容差分别配置。连续性默认报告统计与 URDF 速度辅助指标，不设置机器人相关的通用硬跳变门限。
