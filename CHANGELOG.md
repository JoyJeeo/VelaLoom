...
- **ISSUE-020**：新增 `add_dexhand_tf.py`，使用 `/dexhand/state.position` 和 URDF 运动学把左右手 12 个具名反馈通道转换为 20 条动态指节 TF；增加完整 dry-run、冲突/异常状态门禁、输入保真校验、原子写出和 ROS1/Foxglove 验收流程。
- **ISSUE-021**：在短距离轨迹视觉增强前期研究基础上新增 `loom_xy_motion.py`，可按机器人相对方向、距离和起止时间生成直线 minimum-jerk 水平合成轨迹；通过只改 `odom → base_link.x/y`、完整回读保真和不覆盖原子发布，避免杂乱回环并保护原始 bag。
